...
The Android project includes internet access and inherits the Flutter tool's supported minimum SDK. The iOS/macOS projects include the Keychain Sharing entitlements required by the default secure session store. Flutter bundles `.env` as an application asset, so it is suitable only for public client configuration such as the widget key and API base URL. Never put identity secrets, workspace credentials, or management API credentials in it.
